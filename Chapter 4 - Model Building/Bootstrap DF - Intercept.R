#Bootstrap Method of DF statistic with intercept
...
stat = function(M, y, T, B){
  ...
  G_vec = rep(0,B)
  for(j in 1:B){
    G_vec[j] = G_T_Intercept(M[,j],T)
  }
  # Vector of rep G_T of original
  vec = rep(G_T_Intercept(y,T),length(G_vec))
...
H = quantile(Statistic_g,probs = c(0.01,0.025,0.05,0.95,0.975,0.99)); H
#hist(Statistic_g)
